...
### Sub-step 2 — semantic terracotta audit (no code changes)

**There are no hardcoded terracotta hexes in component files.** The brief asked for a hex grep and a "no hex codes in component files" bar. That bar was already met before this task started — Task 4's consolidation did it. `grep -rniE "#C2410C|#EA580C|#9A3412|#F97316|#FED7AA|#FB923C|#FFF7ED|#7C2D12"` over `app/ components/ lib/ public/` returns four kinds of hit, none of them stylable:

| Location | What it is | Disposition |
|---|---|---|
| `app/globals.css` (31) | The token definitions | Stays — the one file where hexes belong |
| `lib/events/color.ts:47` | `terracotta.hex` swatch value | Stays — palette *data*; the picker paints the circle the user is choosing |
| `public/icon.svg`, `public/favicon.svg` | The logo | Stays — brief: wordmark and logo keep terracotta |
| `cta-band.tsx:6` | A comment mentioning `#FED7AA` | Stays — a comment |

So the real audit is of *semantic* usage, and the useful question is not "which hexes" but "which of these say do-a-thing and which say here's-a-thing".

**Correction to PLAN.md: 86 sites, not 45.** The plan's estimate came from a narrower first-pass grep. The full pattern — `(text|bg|border|border-l|ring|from|via|to)-(brand|primary)(-muted|-hover|-foreground)?` across `app/` and `components/` — returns **86 distinct file:line sites across 41 files**, plus 4 link sites that carry no brand class at all (bare `underline`) and belong in the Q5 population. 90 considered.

**28 lines change; 62 stay.**

#### Swap to blue — 27 lines

| Site | Paints | Why blue |
|---|---|---|
| `sidebar-nav.tsx:65,74` | Active nav strip + icon | Q2 — nav is navigational |
| `reminders/page.tsx:72` | Active filter-tab underline | Same pattern as the sidebar strip, one level down. It's a route-changing `<Link>` |
| `ui/button.tsx:21` | `link` variant | The link variant is *the* prose-link primitive |
| `ui/tooltip.tsx:23` | Tooltip background | Brief names tooltip/hint UI. Gives `--info-foreground` its first real use |
| `auth-layout.tsx:76` | "Sign up" / "Log in" | Prose link |
| `pipeline-strip.tsx:103` | "View all" | Prose link |
| `reminder-bell.tsx:120` | "See all" | Prose link |
| `maps-links.tsx:56,65` | Google / Apple Maps | Prose link (hover state) |
| `j/[slug]/page.tsx:213` | Directions link | Prose link |
| `invite/[token]/page.tsx:55,68` | "Back to home", "Go to login" | Prose links — currently muted/undecorated, no brand class |
| `customer-detail-sheet.tsx:130,135` | `tel:` / `mailto:` | Prose links — currently bare `underline` |
| `extraction-chip.tsx:39,45,46,47` | "Reading…" processing | The machine reporting on itself |
| `extraction-chip.tsx:60` | "Review contract" | Brief-named |
| `extraction-chip.tsx:75` | "AI extracted" (green today) | Brief-named |
| `intake-status-chip.tsx:21,27,28,29` | "Reading…" processing | Mirror of the above |
| `intake-status-chip.tsx:42` | "Ready for review" | Brief-named |
| `intake-status-chip.tsx:57` | "Confirmed" (green today) | Q4(a) — keeps the mirror |

Plus `settings-eta-banner.tsx`, already converted in sub-step 1.

#### One neutral consolidation — 1 line

`ui/skeleton.tsx:9` is `bg-primary/10` — a 10% terracotta wash on every loading placeholder. That is shadcn's stock default, not a decision anyone made here, and a skeleton is neutral chrome by any reading of the split rule. → `bg-muted`. Not blue; zinc.

#### Stays terracotta — 62 lines

Grouped by the reason, since the reason is the reusable part:

- **CTAs and primary actions (11)** — `ui/button.tsx:13` (every Save / Confirm / Add / Send in the product), `ui/badge.tsx:12`, `invite/[token]:88` accept-invite, `orders-table.tsx:271` note-edit affordance, `csv-import-sheet.tsx:394` required-field bullet, `new-order-dialog.tsx:359` "create as new customer", plus the uploader drag-active states (`file-uploader:103`, `intake-uploader:101,118`, `csv-import-sheet:283,286,239`).
- **A choice the user made (13)** — `ui/checkbox.tsx:16`, `ui/calendar.tsx:205` selected day, `orders-filter-bar.tsx:113,155`, `schedule-filter-bar.tsx:255`, `new-order-dialog.tsx:285`, `event-dialog.tsx:358,454,539` selected kind/color/crew, `extraction-review-sheet.tsx:247,253`, `intake-review-sheet.tsx:379,386` selected rows. **The line that separates these from the reminders tab above: a route-changing link is navigation, an in-place filter is a choice.**
- **Brand identity (13)** — topbar wordmark dot, org and user avatars, toast, `empty-state.tsx:65`, and the marketing surfaces (`hero:11`, `feature-grid:61,84`, `built-inside:9,61`, `cta-band:12`, `auth-layout:88,92`).
- **Urgency, which the brief reserves (5)** — `kpi-card.tsx:41,49,57` urgent accents, `install-date.tsx:58` installing-today.
- **Progress / action taken, which the brief reserves (3)** — `pipeline-strip.tsx:64` and `order-stage-badge.tsx:10` fabrication, `quick-add-order-sheet.tsx:210,211` "N orders added this session".
- **Calendar interaction layer (9)** — `calendar-grid.tsx:258,261,267` today header, `:339,473` today column, `:395,474` drop-target, `orders-board.tsx:72,132` drag ring. See the note below.
- **Review-sheet chrome (6)** — step-number circles and the document-type badge in the two review sheets, plus `quick-add-order-sheet:198,309`.
- **Sort-active indicator (1)** — `orders-table.tsx:102`.

#### Three judgment calls worth surfacing

**The calendar's "today" highlight stays terracotta**, and this is load-bearing rather than incidental. "Today" is informational, so the split rule pulls it toward blue — but Fix 2 is about to make event blocks read by color, `delivery` defaults to the blue palette key, and the nav is now blue. A blue today-column behind blue delivery events beside a blue nav is the blue-on-blue muddle flagged as the task's main risk. Keeping the calendar's own interaction layer warm is what keeps the event colors legible. Same reasoning covers the drop-target and drag rings, which are interaction feedback rather than information.

**`topbar.tsx:55` "AI Intake" stays terracotta.** It is a `<Link>`, so it navigates, which argues blue. But it is presented as a button, sits beside "+ New" which the brief explicitly reserves, and turning one of two adjacent topbar buttons blue would split a visual pair for a rule the user cannot see. Presentation beats mechanism here.
